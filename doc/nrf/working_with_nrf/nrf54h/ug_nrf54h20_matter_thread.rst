...
To learn how to perform a DFU using the nRFConnect Device Manager mobile application read instructions in the :ref:`suit smp transfer<nrf54h_suit_sample>` guide.
...
-------------------------------------------------------------
   ...
   #. Run the following command to upgrade the Radio Core:
      ...
      * *peer name* is the the Bluetooth LE name which is advertised by the nRF54H20 PDK (by default ``"Matter Lock"``).
