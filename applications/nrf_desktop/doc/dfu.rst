...
The module can be used for the following SoC series:
...
Writing the data to non-volatile memory, such as flash or MRAM, is triggered when the host performs the fetch operation on the ``sync`` option.
...
For these SoC series, the :ref:`reboot <dfu_reboot>` command after a complete update image transfer triggers an image update performed by the bootloader.
