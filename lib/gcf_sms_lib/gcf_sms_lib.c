...
LOG_MODULE_REGISTER(gcf_sms_lib, CONFIG_GCF_SMS_LIB_LOG_LEVEL);
...
AT_FILTER(CSCS, "AT+CSCS", gcf_sms_lib_filter_callback, PAUSED);
AT_FILTER(CMGD, "AT+CMGD", gcf_sms_lib_filter_callback, PAUSED);
AT_FILTER(CMSS, "AT+CMSS", gcf_sms_lib_filter_callback, PAUSED);
AT_FILTER(CMGW, "AT+CMGW", gcf_sms_lib_filter_callback, PAUSED);
AT_FILTER(CMMS, "AT+CMMS", gcf_sms_lib_filter_callback, PAUSED);
/* Be extra strict on AT+CMGF as implementation for other parameters may be in modem. */
AT_FILTER(CMGF0, "AT+CMGF=0", gcf_sms_lib_filter_callback, PAUSED);
...
struct nrf_modem_at_cmd_filter gcf_sms_lib_filters[] = {
...
#define NUM_INTERNAL_FILTERS (sizeof(gcf_sms_lib_filters) / sizeof(struct nrf_modem_at_cmd_filter))

/*  Check if prefix is identical to start of string. */
static inline bool prefix_match(char *pre, const char *str)
{
...
static nrf_modem_at_cmd_handler_t filter_cb_get(char *at_cmd)
{
	bool match;

	for (size_t i = 0; i < NUM_INTERNAL_FILTERS; i++) {
		match = strstr(at_cmd, gcf_sms_lib_filters[i].cmd);
		if (match) {
			return gcf_sms_lib_filters[i].callback;
		}
	}
 ...
 */
int gcf_sms_lib_filter_callback(char *buf, size_t len, char *at_cmd)
{
	int err = 0;
...
static int at_cmd_callback_cpms(char *buf, size_t len, char *at_cmd)
{
	...
	if (prefix_match("AT+CPMS?", at_cmd)) {
		return at_custom_cmd_response_buffer_fill(
			...
			SMS_BUFFER_ARRAY_SIZE, sms_buffers_used, SMS_BUFFER_ARRAY_SIZE);
	}
...
int _gcf_sms_lib_init(const struct device *unused)
{
	(void)unused;

	return gcf_sms_lib_init();
}

#if defined(CONFIG_GCF_SMS_LIB_SYS_INIT)
/* Initialize during SYS_INIT */
SYS_INIT(_gcf_sms_lib_init, APPLICATION, 1);
#endif
