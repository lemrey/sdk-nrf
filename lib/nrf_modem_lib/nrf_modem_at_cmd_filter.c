...
static int response_buffer_fill(char *buf, size_t len,
		const char *content, ...)
{
	va_list args;
...
static int at_cmd_filter_cmgw(char *buf, size_t len, const char *at_cmd)
{
	...
	if (s2 != NULL) {
		...
		for (int i = 0; i < SMS_BUFFER_LIST_SIZE; i++) {
			...
			/* Store size. */
			sms_buffer_list[i].pdu_size =
					strtol(&(at_cmd[8]), NULL, 10);
			/* Store in SMS buffer. */
			strncpy(sms_buffer_list[i].data, s1,
				(size_t) (s2 - s1));
			/* Make AT response. */
			return response_buffer_fill(buf, len,
					"+CMGW: %d\r\nOK\r\n", i+1);
 ...
 */
int nrf_modem_at_cmd_filter_init(void)
{
	int err;
 ...
 */
int nrf_modem_at_cmd_filter_deinit(void)
{
	int err;
