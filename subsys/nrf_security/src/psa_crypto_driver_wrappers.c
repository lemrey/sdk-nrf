...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_ASYM_SIGN_SUPPORT_CRACEN)
            status = cracen_sign_hash( attributes,
                                       key_buffer,
                                       key_buffer_size,
                                       alg,
                                       hash,
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_ASYM_SIGN_SUPPORT_CRACEN)
            status = cracen_verify_hash(attributes,
                                        key_buffer,
                                        key_buffer_size,
                                        alg,
                                        hash,
...
#endif
#if defined(PSA_NEED_CC3XX_KEY_PAIR_DRIVER)
	   status = cc3xx_import_key(attributes, data, data_length, key_buffer, key_buffer_size,
				     key_buffer_length, bits);
	   /* Declared with fallback == true */
	   if (status != PSA_ERROR_NOT_SUPPORTED) {
	       return status;
...
#endif /* PSA_NEED_CC3XX_KEY_PAIR_DRIVER */
#if defined(PSA_NEED_OBERON_KEY_PAIR_DRIVER)
	   status = oberon_import_key(attributes, data, data_length, key_buffer, key_buffer_size,
				      key_buffer_length, bits);
	   /* Declared with fallback == true */
	   if (status != PSA_ERROR_NOT_SUPPORTED) {
	       return status;
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_ACCEL_KEY_TYPES_CRACEN)
            status = cracen_export_public_key(
                         attributes,
                         key_buffer,
                         key_buffer_size,
                         data,
					       ...
					       size_t output_size, size_t *output_length)
{
	...
	case TFM_BUILTIN_KEY_LOADER_KEY_LOCATION:
		 ...
		 */
            ...
            status = cracen_cipher_encrypt( attributes,
                                        key_buffer,
                                        key_buffer_size,
                                        alg,
                                        iv,
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_CIPHER_SUPPORT_CRACEN)
           status = cracen_cipher_decrypt( attributes,
                                           key_buffer,
                                           key_buffer_size,
                                           alg,
                                           input,
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_CIPHER_SUPPORT_CRACEN)
            status = cracen_cipher_encrypt_setup(
                &operation->ctx.cracen_driver_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
                alg );
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_HAS_CIPHER_SUPPORT_CRACEN)
            status = cracen_cipher_decrypt_setup(
                &operation->ctx.cracen_driver_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
					     ...
					     size_t *hash_length)
{
    ...
    status = cracen_hash_compute(alg, input, input_length, hash, hash_size,
            hash_length);
    if (status != PSA_ERROR_NOT_SUPPORTED)
        return status;
					   ...
					   psa_hash_operation_t *target_operation)
{
	...
	case PSA_CRYPTO_CRACEN_DRIVER_ID:
		...
		return( cracen_hash_clone(
								  &source_operation->ctx.cracen_driver_ctx,
...
#if defined(PSA_CRYPTO_DRIVER_HAS_AEAD_SUPPORT_CRACEN)
            operation->id = PSA_CRYPTO_CRACEN_DRIVER_ID;
            status = cracen_aead_decrypt_setup(
                    &operation->ctx.cracen_driver_ctx,
                    attributes,
                    key_buffer, key_buffer_size,
                    alg );
...
#if defined(PSA_CRYPTO_DRIVER_HAS_AEAD_SUPPORT_CRACEN)
        case PSA_CRYPTO_CRACEN_DRIVER_ID:
            return( cracen_aead_set_lengths(
                    &operation->ctx.cracen_driver_ctx,
					    ...
					    size_t *mac_length)
{
	...
	case TFM_BUILTIN_KEY_LOADER_KEY_LOCATION:
		...
		if (status != PSA_ERROR_NOT_SUPPORTED) {
            ...
            status = cracen_mac_compute(attributes, key_buffer, key_buffer_size, alg,
                input, input_length,
...
#endif /* PSA_NEED_OBERON_MAC_DRIVER */
#if defined(PSA_CRYPTO_DRIVER_HAS_MAC_SUPPORT_CRACEN)
            status = cracen_mac_verify_setup(
                &operation->ctx.cracen_driver_ctx,
                attributes,
                key_buffer, key_buffer_size,
                alg);
...
#if defined(PSA_CRYPTO_DRIVER_HAS_MAC_SUPPORT_CRACEN)
        case PSA_CRYPTO_CRACEN_DRIVER_ID:
            return(cracen_mac_sign_finish(&operation->ctx.cracen_driver_ctx,
                        mac, mac_size, mac_length));
#endif /* PSA_CRYPTO_DRIVER_HAS_MAC_SUPPORT_CRACEN */
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
	default:
...
#if defined(PSA_CRYPTO_DRIVER_HAS_MAC_SUPPORT_CRACEN)
        case PSA_CRYPTO_CRACEN_DRIVER_ID:
            return(cracen_mac_verify_finish(
                        &operation->ctx.cracen_driver_ctx,
                        mac, mac_length));
#endif /* PSA_CRYPTO_DRIVER_HAS_MAC_SUPPORT_CRACEN */
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
					      ...
					      size_t output_size, size_t *output_length)
{
	...
	case TFM_BUILTIN_KEY_LOADER_KEY_LOCATION:
		...
		return status;
            ...
            status = cracen_key_agreement(attributes,
                                          priv_key,
                                          priv_key_size,
                                          publ_key,
                                          publ_key_size,
...
#ifdef PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN
	if (cipher_suite->algorithm == PSA_ALG_SRP_6) {
		status = cracen_srp_setup(
			&operation->ctx.cracen_srp_ctx, cipher_suite);
		if (status == PSA_SUCCESS) {
			operation->id = CRACEN_SRP_DRIVER_ID;
		}
		return status;
	}
#endif /* PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN */
#ifdef PSA_NEED_OBERON_SRP_DRIVER
	if (cipher_suite->algorithm == PSA_ALG_SRP_6) {
...
#endif /* PSA_NEED_OBERON_SPAKE2P_DRIVER */
#ifdef PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN
	case CRACEN_SRP_DRIVER_ID:
		return cracen_srp_set_password_key(
			&operation->ctx.cracen_srp_ctx,
			attributes, password, password_length);
#endif /* PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN */
#ifdef PSA_NEED_OBERON_SRP_DRIVER
...
#ifdef PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN
	case CRACEN_SRP_DRIVER_ID:
		return cracen_srp_get_implicit_key(
			&operation->ctx.cracen_srp_ctx,
			output, output_size, output_length);
#endif /* PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN */
#ifdef PSA_NEED_OBERON_SRP_DRIVER
...
#ifdef PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN
	case CRACEN_SRP_DRIVER_ID:
		return cracen_srp_abort(&operation->ctx.cracen_srp_ctx);
#endif /* PSA_CRYPTO_DRIVER_ALG_SRP_CRACEN */
#ifdef PSA_NEED_OBERON_SRP_DRIVER
	case OBERON_SRP_DRIVER_ID:
