		      ...
		      struct net_buf_simple *buf)
{
	...
	if ((ttl == 1 || ttl > 127) || timeout < 10) {
		LOG_WARN("Invalid params for configuring DM");
			...
			struct net_buf_simple *buf)
{
	...
	if ((tx_ctx.send_ttl == 1 || tx_ctx.send_ttl > 127) || (timeout < 10) || !tx_ctx.addr) {
		LOG_WARN("Invalid params for starting DM");
		err = EBADMSG;
		goto rsp;
	}
	...
	LOG_INFO("Starting %s DM for target node 0x%04x",
...
{
	if (!dm_srv->is_busy) {
		LOG_WARN("Unexpected DM result ready, discarding");
		return;
	}

	LOG_INFO("New DM result ready for target node %04x", dm_srv->target_addr);
