...
static void test_null_pointers(void)
{
	...
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_nordic_unspecified_class_id_get returned (%d)", rc);

	output_size = OUTPUT_MAX_SIZE;
	rc = suit_mci_supported_manifest_class_ids_get(NULL, &output_size);
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_supported_manifest_class_ids_get returned (%d)", rc);
	...
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_downgrade_prevention_policy_get returned (%d)", rc);
	...
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_signing_key_id_validate returned (%d)", rc);

	rc = suit_mci_manifest_class_id_validate(NULL);
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_manifest_class_id_validate returned (%d)", rc);

	rc = suit_mci_processor_start_rights_validate(NULL, processor_id);
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_processor_start_rights_validate returned (%d)", rc);

	rc = suit_mci_memory_access_rights_validate(NULL, mem_address, mem_size);
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_memory_access_rights_validate returned (%d)", rc);

	rc = suit_mci_memory_access_rights_validate(&unsupported_manifest_class_id, NULL, mem_size);
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_memory_access_rights_validate returned (%d)", rc);

	rc = suit_mci_platform_specific_component_rights_validate(NULL,
	...
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_manifest_parent_child_validate returned (%d)", rc);
	...
	zassert_equal(rc, SUIT_PLAT_ERR_INVAL, "suit_mci_vendor_id_for_manifest_class_id_get returned (%d)", rc);
...
static void test_output_buffer_too_small(void)
{
	...
	zassert_equal(
		rc, 0,
		"suit_mci_supported_manifest_class_ids_get returned (%d), isn't %d manifests enough?!",
		rc, OUTPUT_MAX_SIZE);
	...
	if (supported_manifest_count > 1) {
		...
		rc = suit_mci_supported_manifest_class_ids_get(result_uuid, &output_size);
		zassert_equal(rc, SUIT_PLAT_ERR_SIZE, "suit_mci_supported_manifest_class_ids_get returned (%d)",
			      rc);
	}
	...
	zassert_equal(rc, 0, "suit_mci_invoke_order_get returned (%d), isn't %d manifests enough?!", rc,
		      OUTPUT_MAX_SIZE);
	...
	if (invokable_manifest_count > 1) {
		...
		rc = suit_mci_invoke_order_get(result_uuid, &output_size);
		zassert_equal(rc, SUIT_PLAT_ERR_SIZE, "suit_mci_invoke_order_get returned (%d)", rc);
	}
