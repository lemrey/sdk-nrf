...
static const char *trace_start_fmt = "AT%%XMODEMTRACE=1";
static const char *trace_stop_fmt = "AT%%XMODEMTRACE=0";
...
static void nrf_modem_at_printf_ExpectAndReturn(const char *fmt, int retval)
{
	at_printf_fmt_expected = fmt;
	at_printf_return = retval;
}
...
void test_trace_thread_handler_get_single(void)
{
	...
	nrf_modem_at_printf_ExpectTraceLevelAndReturn(trace_level_set_fmt,
						      CONFIG_NRF_MODEM_LIB_TRACE_LEVEL_INIT,
						      0);
...
void test_nrf_modem_lib_trace_resume(void)
{
	int ret;

	nrf_modem_at_printf_ExpectAndReturn(trace_start_fmt, 0);

	ret = nrf_modem_lib_trace_resume();
	TEST_ASSERT_EQUAL(0, ret);
}

void test_nrf_modem_lib_trace_resume_efault(void)
{
	int ret;

	nrf_modem_at_printf_ExpectAndReturn(trace_start_fmt, -EFAULT);

	ret = nrf_modem_lib_trace_resume();
	TEST_ASSERT_EQUAL(-ENOEXEC, ret);
}

void test_nrf_modem_lib_trace_pause(void)
{
	int ret;

	nrf_modem_at_printf_ExpectAndReturn(trace_stop_fmt, 0);

	ret = nrf_modem_lib_trace_pause();
	TEST_ASSERT_EQUAL(0, ret);
}

void test_nrf_modem_lib_trace_pause_efault(void)
{
	int ret;

	nrf_modem_at_printf_ExpectAndReturn(trace_stop_fmt, -EFAULT);
