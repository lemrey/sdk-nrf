...
/** @brief Set trace level and resume a trace session, if paused.
 *
 * This function sends an AT command that sets the trace level and resumes tracing.
 *
...
/** @brief Resume the trace session.
 *
 * This function resumes the trace session.
 *
 * @return Zero on success, non-zero otherwise.
 */
int nrf_modem_lib_trace_resume(void);

/** @brief Pause the trace session.
 *
 * This function pauses the trace session.
 *
 * @return Zero on success, non-zero otherwise.
 */
